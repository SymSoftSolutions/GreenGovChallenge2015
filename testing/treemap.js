...
var margin = {
  ...
  width =    document.querySelector('#attach').clientWidth,
...
var svg = d3.select( "#attach" ).append( "svg" )
...
function convert( root, wantedValue ) {
  ...
  var newNest = {
    ...
    "children": nest.map( function ( department ) {

        return {
          "name": department.key,
          "children": department.values.map( function ( propType ) {
              // console.log( propType.values.length );
              return {
                "name": propType.key,
                "children":

                  propType.values.map( function ( location ) {
                    var value = Number.parseFloat( location.values[ 0 ]
                      [ wantedValue ] )
                  ...
                  }) //end of map(function(location){
              };

            } ) //end of map(function(propType){
        };

      } ) //end of map(function(department){
  }; //end of var declaration
  console.log( newNest );
  return newNest
}

// d3.csv(BuildingMetrics2013, function(root) {
//   var newRoot = convert(root);
// })

// d3.json(DeparmentThenPropType, function(root){
//   var root = convertDepartment(root);
// })
